...
model_file = os.path.join(os.path.abspath("."), 'best_policy_value_net_4400.pth')
...
def simulate_game(head_start, game_round, thread_id):
    ...
    board = ChessBoard()  # 每次模拟一局游戏，都重新初始化一个board
    ...
    if head_start == TREE_PLAYER:
        ...
        while not board.is_game_over()[0]:  # 游戏是否结束
            ...
            _board = np.full(shape=(9, 9), fill_value=-1)
            for action in board.state:
                _board[action // 9][action % 9] = board.state[action]
            print(_board)
...
def run_games(num_games_per_process, result_queue, thread_id):
    ...
    result_queue.put((tree_wins, graph_wins, tree_wins_step, graph_wins))
