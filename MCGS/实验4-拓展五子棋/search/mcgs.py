...
class AlphaZeroMCGS:
    ...
    def get_action(self, board0: ChessBoard) -> Union[Tuple[int, np.ndarray], int]:
        self.step += 1
        self.iter_num = 0
    ...
    def select_and_expand(self, root_node_id: int, board0: ChessBoard) -> Union[Tuple[int, np.ndarray], int]:
        board = board0.copy()
        if self.iter_num == 5:
            print()
        ...
        self.expansion(current_node_id, board.state, board.available_actions, p, board.current_player)
    ...
    def expansion(self, parent_id, board0, action:list, p, player):
        for idx in range(len(action)):
            board0[action[idx]] = player
            # board0[int(action[idx] // 9)][int(action[idx] % 9)] = player
            ...
            del board0[action[idx]]
            # board0[int(action[idx] // 9)][int(action[idx] % 9)] = -1  # -1代表EMPTY
