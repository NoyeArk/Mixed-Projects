...
import copy
from run_mcgs import UCT_search, do_decode_n_move_pieces, get_policy
import pickle
...
class arena():
    ...
    def play_round(self):
        ...
        while checkmate == False and current_board.actions() != []:
            ...
            print(""); print(current_board.board)
            if current_board.current_player == WHITE:
                root = UCT_search(current_board,777,white,t)
                policy = get_policy(root, t); print("Policy: ", policy, "white = %s" %(str(w)))
            elif current_board.player == BLACK:
...
def evaluate_nets(args, iteration_1, iteration_2) :
    ...
    logger.info("当前的神经网络: %s" % current_net)
    logger.info("之前(最好)的神经网络: %s" % best_net)
    ...
    if args.MCGS_num_processes > 1:
        ...
        processes = []
        if args.MCGS_num_processes > mp.cpu_count():
            num_processes = mp.cpu_count()
            logger.info("Required number of processes exceed number of CPUs! Setting MCGS_num_processes to %d" % num_processes)
        else:
    ...
    elif args.MCGS_num_processes == 1:
        current_cnet.eval(); best_cnet.eval()
        checkpoint = torch.load(current_net_filename)
        current_cnet.load_state_dict(checkpoint['state_dict'])
        checkpoint = torch.load(best_net_filename)
