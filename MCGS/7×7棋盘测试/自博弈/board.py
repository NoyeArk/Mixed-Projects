...
class Board:
    ...
    def __init__(self, board_len=7, n_feature_planes=2):
        """
        初始化棋盘。
    ...
    def dfs(self, board, start: int, end: int):
        ...
        """
        q = []
        dfs_air_visit = np.full(shape=(self.board_len, self.board_len), fill_value=False)
        q.append(start)
    ...
    def is_over(self, board, color):
        ...
        :return: 是否红方胜利。
        """
        if color not in board[0] or color not in board[self.board_len-1]:
            return False
        start = []
        end = []
        for i in range(self.board_len):
            if board[0][i] == color:
                start.append(i)
            if board[self.board_len-1][i] == color:
                end.append((self.board_len-1) * self.board_len + i)
        for i in start:
            for j in end:
    ...
    def is_game_over(self):
        ...
        *int ->游戏的赢家。
                ...
                **如果游戏未分出胜负，None
        """
        if len(self.state) < self.board_len:
            return False, None
