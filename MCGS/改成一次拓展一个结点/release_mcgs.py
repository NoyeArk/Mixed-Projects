...
class AlphaZeroMCTS:
    ...
    def search_init(self):
        ...
        self.G.add_node(0, pi=1, parent=0, N_s=0, Q_s=0, hash=0,
                        chess_num=0, child_pi=None, available_action=None)
    ...
    def get_action(self, board0: Board) -> Union[Tuple[int, np.ndarray], int]:
        ...
        if key in self.hash_table.keys():
            ...
            print('没有更换根节点, key是', key)

        if (self.root_node_id == 0):
            self.G.nodes[self.root_node_id]['available_action'] = board0.available_action
            p, value = self.net.predict(board0)
            self.G.nodes[self.root_node_id]['Q_s'] = value

            child_pi = dict(zip(board0.available_action, p))
            self.G.nodes[self.root_node_id]['child_pi'] = child_pi
            self.G.nodes[self.root_node_id]['board'] = board0.board
            # print(self.G.nodes[self.root_node_id]['child_pi'])
        ...
        self.logger.info('当前节点数：%s 当前边数：%s', self.G.number_of_nodes(), self.G.number_of_edges())

        children_id = list(self.G.neighbors(self.root_node_id))  # 所有的孩子节点
        sorted_children_id = sorted(children_id, key=lambda child: self.G.edges[(self.root_node_id, child)]['action'])

        for child in sorted_children_id:
            print('对应的动作:', self.G.edges[(self.root_node_id, child)]['action'])
            print('对应局面的剩余动作:', self.G.nodes[child]['available_action'], end='')
            print('对应局面的哈希值:', self.G.nodes[child]['hash'])
            print('对应的棋盘:')
            print(self.G.nodes[child]['board'])

        print('根节点对应的局面')
        print(self.G.nodes[self.root_node_id]['board'])
        print(board0.board)
        # 删除无用节点
        ...
        self.logger.info('累计剪枝节点数：%s 累计剪枝边数：%s', self.prune_node_num, self.prune_edge_num)
    ...
    def select_and_expand(self, root_node_id: int, board0: Board) -> Union[Tuple[int, np.ndarray], int]:
        ...
        while not is_leaf: # 要么选到叶子节点 要么是TN结点
            next_node_id, is_leaf = self.selection(current_node_id, board)
    ...
    def selection(self, parent_id, board0):
        board = board0.copy()
        children_id = list(self.G.neighbors(parent_id)) # 所有的孩子节点

        if (len(children_id) == len(self.G.nodes[parent_id]['available_action'])): # 所有孩子结点全部拓展
            # 选择出一个最好的孩子结点
            best_node_id = max(children_id, key=lambda id: self.uct_value(id, parent_id))
            assert nx.has_path(self.G, parent_id, best_node_id)
            return best_node_id, False

        else: # 未被全部拓展，在剩下的可行动作中随机选取一个动作，同时生成一个结点
            random_action_pi = random.choice(list(self.G.nodes[parent_id]['child_pi'].items()))
            del self.G.nodes[parent_id]['child_pi'][random_action_pi[0]]
            action = random_action_pi[0]
            pi = random_action_pi[1]

            board.do_action(action)
            # print(board.board)
            _hash, chess_num = compute_zobrist_hash(board.board)

            if _hash in self.hash_table.keys():  # 该节点已经存在
                self.tn_times_hash += 1
                self.G.add_edge(parent_id, self.hash_table[_hash], action=action, N_s_a=0, Q_s_a=0)

                return self.hash_table[_hash], False

            else:  # 创建新节点
                # print(len(board0.get_available()))
                self.node_count += 1
                new_node_id = self.node_count
                # 创建新结点
                self.G.add_node(new_node_id, pi=pi, N_s=0, Q_s=0, hash=_hash, board=board.board,
                                chess_num=chess_num, child_pi=None, available_action=board.available_action)
                # 创建一条边
                self.G.add_edge(parent_id, new_node_id, action=action, N_s_a=0, Q_s_a=0)
                # 将其添加到在线维护的哈希表中
                self.hash_table[_hash] = new_node_id

                board0.board[int(action // 11)][int(action % 11)] = -1  # -1代表EMPTY

                return new_node_id, True

    def expansion(self, leaf_id, board0, action:list, p, player):

        child_pi = dict(zip(action, p))
        self.G.nodes[leaf_id]['child_pi'] = child_pi
