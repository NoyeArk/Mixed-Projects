...
class AlphaZeroMCTS:
    ...
    def __init__(self, net, c_puct: float = 1, type=NODE, n_iters=100.0, game_round=0, thread_id=0) -> None:
        self.use_mcgs = True
        self.c_puct = c_puct
        self.n_iters = 1000 # 如果是NODE，则为节点数，如果是TIME，则为搜索时间
        self.net = net
        self.type = type
    ...
    def get_action(self, board0: Board) -> Union[Tuple[int, np.ndarray], int]:
        ...
        if key in self.hash_table.keys() and self.use_mcgs:
            ...
            children_id = list(self.G.neighbors(self.root_node_id))
            print(len(children_id))
            # self.hash_table = {key: value for key, value in self.hash_table.items() if key in children_id}

            self.calculate_effective_nodes(self.root_node_id)
            # self.delete_all_node(self.root_node_id)
        ...
        # self.delete_all_node(children[actions.index(action)])
    ...
    def delete_all_node(self, start_node):
        # 找到从起始节点出发的所有联通节点
        connected_nodes = nx.descendants(self.G, start_node)
        connected_nodes.add(start_node)  # 包括起始节点

        # 创建一个子图，仅包括从起始节点出发的节点和相关边
        subgraph = self.G.subgraph(connected_nodes)

        # 删除除了子图中节点之外的其他节点和相关边
        nodes_to_remove = set(self.G.nodes) - set(subgraph.nodes)
    ...
    def calculate_effective_nodes(self, start_node):
        ...
        # # 找出从起始节点出发的弱连通组件（联通的节点集合）
        # weakly_connected = list(nx.strongly_connected_components(self.G))
        #
        # # 查找包含起始节点的弱连通组件
        # for component in weakly_connected:
        #     if start_node in component:
        #         print(f"从节点 {start_node} 出发能够联通的节点数: {len(component)}")
        #         break
        #
        # # 找到从起始节点出发的所有联通节点
        # connected_nodes = nx.descendants(self.G, start_node)
        ...
        #     connected_edges += self.G.in_degree(node)  # 计算节点的入度
        #
        # print(f"从节点 {start_node} 出发能够联通的边数: {connected_edges}")
    ...
    def uct_value(self, child_id, parent_id):
        ...
        try:
            ...
            print('入度为:', self.G.in_degree[child_id])
            print(child_id)
            print(self.G.nodes[child_id])
