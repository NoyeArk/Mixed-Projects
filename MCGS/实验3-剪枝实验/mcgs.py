...
class AlphaZeroMCGS:
    ...
    def predict(self, board):
        ...
        feature_planes_np = board.get_feature_planes().numpy().astype(np.float32)
        expanded_feature_planes = np.expand_dims(feature_planes_np, axis=0)
        expanded_feature_planes[0] = 1
        # 准备输入数据（根据模型的输入要求）
        input_name = self.net.get_inputs()[0].name
        result = self.net.run([], {input_name: expanded_feature_planes})
    ...
    def select_and_expand(self, root_node_id: int, board0: Board) -> Union[Tuple[int, np.ndarray], int]:
        ...
        p, value = self.predict(board)
        self.expansion(current_node_id, board.board, board.available_action, p[0], board.current_player)
        return trajectory, value[0], current_node_id
