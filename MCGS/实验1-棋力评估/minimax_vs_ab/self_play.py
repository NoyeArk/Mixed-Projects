...
model = PolicyValueNet(11, 2, 20, 128, True)
model_file = os.path.join(os.path.abspath("."), 'model/20b128c_hex11.pth')
model_dict = torch.load(model_file, map_location='gpu').state_dict()
...
def run_games(num_games_per_process, result_queue, thread_id):
    ...
    with tqdm(total=num_games_per_process, unit="game") as pbar:
        ...
        if loop % 100 == 0:
            ...
            logger.info("minimax wins:%s", graph_wins)
...
if __name__ == "__main__":
    ...
    logger.info("minimax wins:%s", total_graph_wins)
    logger.info("ab wins/step:", total_tree_wins_step / total_tree_wins)
    logger.info("minimax wins/step:", total_graph_wins_step / total_graph_wins)
