...
from search.gmcgs import AlphaZeroGMCGS
from search.ab import AlphaBeta
from logger import logger_config

logger = logger_config(log_path='outcome.log', logging_name='ab VS minimax 比赛结果记录')
...
def simulate_game(head_start, game_round, thread_id):
    ab = AlphaBeta(net=model, max_node=SEARCH_NODE)
    gmcgs = AlphaZeroGMCGS(net=model, c_puct=1, type=NODE, n_iters=SEARCH_NODE, game_round=game_round, thread_id=thread_id)

    board = Board()  # 每次模拟一局游戏，都重新初始化一个board

    if head_start == TREE_PLAYER:
        while not board.is_game_over()[0]: # 没有结束
            board.do_action(ab.get_action(board))
            print_game_info() # 打印步数信息

            if board.is_game_over()[0]: # 如果游戏结束
                return TREE_WIN

            board.do_action(gmcgs.get_action(board))
            print_game_info()
...
def run_games(num_games_per_process, result_queue, thread_id):
    ...
    with tqdm(total=num_games_per_process, unit="game") as pbar:
        ...
        if loop % 100 == 0:
            logger.info("ab wins:%s", tree_wins)
            logger.info("gmcgs wins:%s", graph_wins)
...
if __name__ == "__main__":
    ...
    print("ab wins:", total_tree_wins)
    print("gmcgs wins:", total_graph_wins)
    print('ab wins/step:',total_tree_wins_step / total_tree_wins)
    print('gmcgs wins/step:',total_graph_wins_step / total_graph_wins)

    logger.info("ab wins:%s", total_tree_wins)
    logger.info("gmcgs wins:%s", total_graph_wins)
    logger.info("ab wins/step:", total_tree_wins_step / total_tree_wins)
    logger.info("gmcgs wins/step:", total_graph_wins_step / total_graph_wins)
