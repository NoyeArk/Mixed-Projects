...
def game_end(board):
    ...
    # row_list.append((board.b_0_3 & np.uint64(8585740288)) >> np.uint64(
    #     11 * 2 - 2))  # 8585740288 11111111111 00000000000 00000000000
    # row_list.append((board.b_0_3 & np.uint64(17583596109824)) >> np.uint64(
    #     11 * 3 - 3))  # 17583596109824 11111111111000000000000000000000000000000000
    #
    ...
    loop = 7
    print(row_list[loop])
    print(row_list[loop + 1])
    print(row_list[loop + 2])
    print(row_list[loop + 3])
    for loop in range(8):
        outcome_and = row_list[loop] & row_list[loop + 1] & row_list[loop + 2] & row_list[loop + 3]
        if (bin(outcome_and).count('1')) >= 1:
            return END
